...
#从测试集分25%作为验证集
x_val, x_test, y_val, y_test = train_test_split(x_test, y_test, test_size=0.99)
#%%
...
device='cuda:0'
model = transtab.build_classifier(binary_columns=binary_columns, numerical_columns=numerical_columns, 
...
plt.show()
#打印训练集准确率
train_pred = transtab.predict(clf = model, x_test = train_x, y_test=train_y)
