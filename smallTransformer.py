...
df = pd.read_csv('D:\Desktop\Explain_CEC_Recording\data\com_patient_sample_mrna.csv', header=None, index_col=0, low_memory=False)
data_raw = df.T
data_raw = data_raw.dropna(axis=1, how='any')
data_raw = data_raw.drop(['P_ID'], axis=1)
data_raw['label'] = pd.to_numeric(data_raw['label'], errors='coerce')
data_raw = pd.DataFrame(data_raw, dtype=np.float32)

feature = data_raw.drop(['label'], axis=1)
label = data_raw['label'].values
targets = label
...
class Transformer(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim, num_layers, num_heads, dropout):
        ...
        # 使用局部注意力,不缓存attention权重
        self.encoder_layer.score_attn_weights = False
...
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # 创建数据集和数据加载器
...
exp = explainer.explain_instance(x_test_np[5], model.predict_proba, num_features=num_features, top_labels=1,
